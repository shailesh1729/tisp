...
1. A *projective space* corresponding to a vector space $V$
...
1. A $D$-dimensional subspace $V$ can be defined by $D' = M - D$
   linearly independent linear forms $\{b_1, b_2, \dots, b_{D'} \}$:

    $$
    V = \{\bx \in \RR^M | b_i(\bx) = 0,  1 \leq i \leq D' \}.
    $$
1. Let $V^*$ denote the vector space of all linear forms
   that vanish on $V$.
1. Then $\dim(V^*) = D' = M - D$. 
1. $V$ is the zero set of $V^*$ (i.e. $V = Z(V^*))$.
1. The vanishing ideal of $V$ is

    $$
    I(V) = \{ p(\bx) \in \RR[\bx] : p(\bx) = 0, \Forall \bx \in V \}.
    $$
1. $I(V)$ is an ideal generated by linear forms in $V^*$.
1. It contains polynomials of all degrees that vanish on $V$.
1. Every polynomial $p(\bx) \in I(V)$ can be written as

    $$
    p(\bx) = h_1 b_1 + \dots  + h_{D'} b_{D'}
    $$
    where $h_i \in \RR[\bx]$.
1. $I(V)$ is a prime ideal. 
